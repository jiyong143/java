...
;
		...
		/* -list3는 list 인터페이스를 구현한 구현 클래스들의 객체 중 아무나 올 수 있다
		 * 
		 *  - list3 = new ArrayList<String>(); //o
		 *  - lisr3 = new LinkedList<String>();//o
		 * 
		 ...
		 *  - 이때, Object.equals 를 이용해 동작
		 *  - 컬렉션에 있는 타입과 객체의 타입이 다르면 false 리턴
		 *  - 같으면 객체.equals() 를 호출해 결과 리턴
		 *  - 해당 클래스의 equals가 어떻게 오버라이딩 되어 있는냐 따라 결과 다름
		 */
		
		System.out.println(list5.contains("abc"));
		/* indexOf(객체) : List, 리스트에 객체가 몇번지에 있는지 알랴줌, 없으면 -1
		 * - 동작 방식은 contains 일치
		 */
		 ...
		 * remove(객체) : Collection, 객체와 일치하는 요소를 제거 후 제거 여부 반환
		 */
		...
		/* iterator() : Collection, 컬렉션을 이용해서 반복자를 생성해서 리턴
		 * => 반복자를 이용해서 반복문으로 활용
		 */
		...
		/*sort() : List, 람다식을 이용하여 정렬기준을 정해주면 정해진 정렬기준으로 리스트를 정렬
		 * - 정렬 기준은 정수로 알려주고, 음수이면 앞, 0이면 같은 위치 양수이면 뒤 
		 * - Compator 인터페이스를 구현한 클래스의 객체가 필요
		 * - 일반적으로 람다식을 이용하여 익명 객체를 생성해서 활용*/
		
		list5.sort((s1,s2)->s1.compareTo(s2));
		
		/*Collection.sort() : 리스트의 타입이 comparable 인터페이스를 구현한 구현 클래스인 경우
		 * 해당 기준을 이용해서 정렬
		 */
