...
public class School {
	...
	public boolean professorLessonEquals(int classOf) {
		if(lessonList.contains(new Lesson(classOf))) {
			return true;
		}
		return false;
	...
	public void selectLessonStudent(int classOf) {
		...
		if(index>=0) {//강의가 있는지 없는지
			...
			for(Integer tmp:cl){
				index=studentList.indexOf(new Student(tmp));
	...
	public void addScore(int classOf, int score,int num) {
		...
		int index=lessonList.indexOf(new Lesson(subject));//강의이름을 통해 해당리스트의 인덱스를 가져옴
		if(lessonList.get(index).getScoreList().size()==0) {//태스트
	...
	//강의 등록/교수는 수업한개만하도록 수정 : 병호님이 만든 addLesson 메서드를 축약? 해서 만들어봄...
	
	public void addLesson1(String subject, int professor, int classroom, String dayOfWeek, int sTime, int eTime, int max) {
		
		// 강의명이 기존에 있거나 강의시간과 장소가 겹치거나 , 없는 교번이거나 수업에 이미 들어가는 교번이면 수업 등록 불가
		
		 //교번에 문제가 있는 경우
		if(!professorEquals(professor)||professorLessonEquals(professor)) {
			System.out.println("없는 교번이거나 해당 교번은 이미 등록된 수업이 있습니다.");
			return;
		}
		// 강의명이나 강의실, 시간에 문제가 있는 경우
		for(Lesson tmp : lessonList) {
			...
			System.out.println("수업 등록이 완료됐습니다.");
		}
	...
	public void addLesson(String subject, int professor, int classRoom, String dayOfWeek, int sTime, int eTime,
			int max) {
		// 교번이 있어야하며, 강의 리스트에 없는 교번이여야 한다.
		if(lessonList.size()==0) {
			lessonList.add(new Lesson(subject, professor, dayOfWeek, sTime, eTime, classRoom, max));
			System.out.println(lessonList);//태스트
			return;
		}
		String suject2=LessonName(professor);
		if(suject2!=null) {
		...
		if(lessonList.contains(new Lesson(subject))) {
			System.out.println("같은 강의명이 있습니다");
			return;
		}
		//요일 시간 강의실 비교
		for(Lesson tmp:lessonList) {
	...
	public void runSetLessonMenu(int menu,int index) {
		...
		case 2 :
			...
			boolean t=true;
			if((professorEquals(newProfessor))&&(!professorLessonEquals(newProfessor))){
